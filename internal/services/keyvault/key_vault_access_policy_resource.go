...
func keyVaultAccessPolicy() *schema.Resource {
	return &schema.Resource{
		Create: keyVaultAccessPolicyCreate,
		Read:   keyVaultAccessPolicyRead,
		Update: keyVaultAccessPolicyUpdate,
		Delete: keyVaultAccessPolicyDelete,
...
func keyVaultAccessPolicyCreateOrDelete(d *schema.ResourceData, meta interface{}, action keyvault.AccessPolicyUpdateKind) error {
	...
	id, err := resourceids.ParseAzureResourceID(vaultId)
	if err != nil {
		return err
	}
	...
	if d.IsNewResource() {
		...
		for _, policy := range *props.AccessPolicies {
			...
			if tenantIdMatches && objectIdMatches && applicationIdMatches {
				return tf.ImportAsExistsError("azurestack_key_vault_access_policy", resourceId)
			}
		}
	}
...
func keyVaultAccessPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	return keyVaultAccessPolicyCreateOrDelete(d, meta, keyvault.Add)
}

func keyVaultAccessPolicyDelete(d *schema.ResourceData, meta interface{}) error {
	return keyVaultAccessPolicyCreateOrDelete(d, meta, keyvault.Remove)
}

func keyVaultAccessPolicyUpdate(d *schema.ResourceData, meta interface{}) error {
	return keyVaultAccessPolicyCreateOrDelete(d, meta, keyvault.Replace)
}

func keyVaultAccessPolicyRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).KeyVault.VaultsClient
