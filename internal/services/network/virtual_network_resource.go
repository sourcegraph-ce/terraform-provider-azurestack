...
func virtualNetworkCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if !utils.ResponseWasNotFound(existing.Response) {
			return tf.ImportAsExistsError("azurestack_virtual_network", id.ID())
		}
	}
...
func expandazurestackVirtualNetworkVirtualNetworkSecurityGroupNames(d *pluginsdk.ResourceData) ([]string, error) {
