...
func loadBalancerProbe() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		...
		Delete: loadBalancerProbeDelete,
...
func loadBalancerProbeCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	newProbe := expandazurestackLoadBalancerProbe(d)
	...
	if exists {
		if id.ProbeName == *existingProbe.Name {
			if d.IsNewResource() {
				return tf.ImportAsExistsError("azurestack_lb_probe", *existingProbe.ID)
			}
...
func loadBalancerProbeRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).LoadBalancer.LoadBalancersClient
...
func loadBalancerProbeDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).LoadBalancer.LoadBalancersClient
...
func expandazurestackLoadBalancerProbe(d *pluginsdk.ResourceData) *network.Probe {
