...
func loadBalancerNatRule() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: loadBalancerNatRuleCreateUpdate,
		Read:   loadBalancerNatRuleRead,
		Update: loadBalancerNatRuleCreateUpdate,
		Delete: loadBalancerNatRuleDelete,
...
func loadBalancerNatRuleCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	newNatRule, err := expandazurestackLoadBalancerNatRule(d, &loadBalancer, *loadBalancerId)
	if err != nil {
	...
	if exists {
		if id.InboundNatRuleName == *existingNatRule.Name {
			if d.IsNewResource() {
				return tf.ImportAsExistsError("azurestack_lb_nat_rule", *existingNatRule.ID)
			}
...
func loadBalancerNatRuleRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).LoadBalancer.LoadBalancersClient
...
func loadBalancerNatRuleDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).LoadBalancer.LoadBalancersClient
...
func expandazurestackLoadBalancerNatRule(d *pluginsdk.ResourceData, lb *network.LoadBalancer, loadBalancerId parse.LoadBalancerId) (*network.InboundNatRule, error) {
